// Mock of MapView with <AIRMap onPress={undefind} /> 
// to allow Detox .tap() on Marker
...
const propTypes = {
  ...
  /**
   * The LatLng threshold at which the map should switch between city pins and provider pins
   * Default value is `360`
   *
   * @platform android
   */
  switchToCityPinsDelta: PropTypes.number,

  /**
   * The pins to display on the map when zoomed out
   *
   * @platform android
   */
  cityPins: PropTypes.arrayOf(
    PropTypes.shape({
      pos: PropTypes.shape({
        /**
         * Coordinates for the location of the city pin.
         */
        latitude: PropTypes.number.isRequired,
        longitude: PropTypes.number.isRequired,
      }).isRequired,
  ...
  /**
   * The token to pass to the URBI server.
   *
   * @platform android
   */
  backendToken: PropTypes.string,

  /**
   * Display paths to selected pins if reaching them by foot takes shorter than this (in seconds)
   *
   * @platform android
   */
  showPathIfCloserThanSeconds: PropTypes.number,

  /**
   * Android-only. If set, the GoogleMap instance held by this map will be cleared and dismissed
   * every time another MapView with singleInstance set is initialized.
   * Default value is `false`
   *
   * @platform android
   */
  singleInstance: PropTypes.bool,
  ...
  /**
   * Callback that is called when a city pin on the map is tapped by the user.
   *
   * @platform android
   */
  ...
  /**
   * Callback that is called when the user moves to or from a city.
   *
   * @platform android
   */
  ...
  /**
   * Callback that is called when location permissions should be requested to the user.
   *
   * @platform android
   */
  onPermissionsNeeded: PropTypes.func,
   ...
   * the callout for that marker is about to be hidden./center
   *
   * @platform ios
   */
  ...
  /**
   * Callback that is called when the location gets updated by the location services.
   *
   * @platform android
   */
  ...
  /**
   * Callback that is called when the user either accepts or denies the request to
   * enable location services on their device.
   *
   * @platform android
   */
  onLocationServicesEnableResponse: PropTypes.func,
};
...
class MapView extends React.Component {
  constructor(props) {
    ...
    this._onCityPress = this._onCityPress.bind(this);
  ...
  _onCityPress(event) {
    if (this.props.onCityPress) {
      this.props.onCityPress(event.nativeEvent);
    }
  }
  ...
  getLastLocation() {
    if (Platform.OS === 'android') {
      return NativeModules.AirMapModule.getLastLocation(this._getHandle());
    } else if (Platform.OS === 'ios') {
      return Promise.reject('getLastLocation not supported on this platform');
    }
  }
  ...
  centerToUserLocation(fromButtonPress) {
    this._runCommand('centerToUserLocation', [fromButtonPress]);
  }

  centerTo(coordinates) {
    this._runCommand('centerTo', [coordinates]);
  }

  setRadarCircle(center, radius) {
    this._runCommand('setRadarCircle', [center, radius]);
  }

  hideRadarCircle() {
    this._runCommand('hideRadarCircle');
  }
  ...
  _getHandle() {
    const handle = findNodeHandle(this.map);
    if (handle) {
